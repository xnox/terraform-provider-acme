	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"account_key_pem": {
				...
				ForceNew:     true,
				Sensitive:    true,
				ExactlyOneOf: []string{"account_key_pem", "account_key_pem_wo"},
			},
			"account_key_pem_wo": {
				// Write-only variant of account_key_pem. The value is supplied in
				// the configuration but never written to state, allowing the
				// account key to be sourced from an ephemeral resource (e.g.
				// ephemeral.google_secret_manager_secret_version) without
				// persisting it. Requires Terraform 1.11 or later.
				//
				// Note that this key is only available to the provider during
				// plan/apply, so it is used for issuance, renewal and revocation
				// (which happen at apply). Renewal *detection* (ARI / expiry) runs
				// during refresh and plan without it - see expandACMECertifierForRead.
				Type:         schema.TypeString,
				Optional:     true,
				WriteOnly:    true,
				Sensitive:    true,
			...
			"account_key_pem_wo_version": {
				...
				// never stored, Terraform cannot detect when it changes. Bumping
				// this version is how a practitioner signals an account key
				// rotation: a change here forces an in-place renewal, re-issuing
				// the certificate under the (new) write-only account key. This is a
				// renewal, not a replacement, so the previous certificate is not
				// revoked (a write-only key cannot be used to revoke on destroy -
				// see the guard in resourceACMECertificateCustomizeDiff).
				Type:         schema.TypeInt,
				Optional:     true,
			...
			"key_type": {
				...
				ForceNew:      true,
				Default:       "RSA2048",
				ConflictsWith: []string{"certificate_request_pem", "private_key_pem_wo"},
				ValidateFunc:  validateKeyType,
			},
			"private_key_pem_wo": {
				// Write-only private key used to issue the certificate. Supplied in
				// the configuration (e.g. from ephemeral.tls_private_key) but never
				// written to state. When set, the certificate is issued using this
				// key instead of one generated by the provider, and - as with
				// certificate_request_pem - the private_key_pem and certificate_p12
				// attributes are left empty (the key is never persisted). Requires
				// Terraform 1.11 or later.
				Type:          schema.TypeString,
				Optional:      true,
				WriteOnly:     true,
				Sensitive:     true,
				ConflictsWith: []string{"certificate_request_pem", "key_type"},
			},
			"private_key_pem_wo_version": {
				// Companion to private_key_pem_wo. Because the write-only value is
				// never stored, Terraform cannot detect when it changes. Increment
				// this integer to rotate the certificate's private key; a change
				// forces a new resource, re-issuing the certificate under the new
				// key.
				Type:         schema.TypeInt,
				Optional:     true,
				ForceNew:     true,
			...
			"certificate_request_pem": {
				...
				ForceNew:      true,
				AtLeastOneOf:  []string{"common_name", "subject_alternative_names", "certificate_request_pem"},
				ConflictsWith: []string{"common_name", "subject_alternative_names", "key_type", "private_key_pem_wo"},
			},
			"validity_days": {
				Type:         schema.TypeInt,
				Optional:     true,
...
func resourceACMECertificateCreate(d *schema.ResourceData, meta any) error {
	...
	if v, ok := d.GetOk("certificate_request_pem"); ok {
		...
		// If a write-only private key was supplied, issue the certificate with it
		// rather than letting lego generate one. The key is not persisted.
		if woKey := writeOnlyValue(d, "private_key_pem_wo"); woKey != "" {
			obtainReq.PrivateKey, err = privateKeyFromPEM([]byte(woKey))
			if err != nil {
				return err
			}
		}

		cert, err = client.Certificate.Obtain(context.TODO(), obtainReq)
	}
	...
	// When the private key was supplied write-only, do not persist it: clear it
	// so private_key_pem and certificate_p12 are left empty, as with a CSR.
	if writeOnlyValue(d, "private_key_pem_wo") != "" {
		cert.PrivateKey = nil
	}
...
func resourceACMECertificateRead(d *schema.ResourceData, meta any) error {
	...
	// The ARI / renewal-info refresh is an unauthenticated GET and needs no
	// account key, so it works during refresh even when a write-only key is in
	// use.
	certifier, err := expandACMECertifierForRead(d, meta)
	if err != nil {
		return err
	}

	if err := resourceACMECertificateRenewalInfoRefresh(d, certifier, time.Now()); err != nil {
		return err
	}
...
func resourceACMECertificateCustomizeDiff(_ context.Context, d *schema.ResourceDiff, meta any) error {
	...
	// A write-only account key (account_key_pem_wo) cannot be used to revoke the
	// certificate on destroy: Terraform provides no configuration - and thus no
	// write-only values - during a destroy, so the key is unavailable when Delete
	// runs. Fail fast at plan time rather than erroring during destroy.
	if d.Get("revoke_certificate_on_destroy").(bool) {
		raw := d.GetRawConfig()
		if !raw.IsNull() && raw.Type().IsObjectType() && raw.Type().HasAttribute("account_key_pem_wo") &&
			!raw.GetAttr("account_key_pem_wo").IsNull() {
			return fmt.Errorf("revoke_certificate_on_destroy must be set to false when " +
				"account_key_pem_wo is used: a write-only account key is not available during " +
				"destroy, so the certificate cannot be revoked")
		}
	}
	...
	// A change to account_key_pem_wo_version signals an account key rotation.
	// Force an in-place renewal so the certificate is re-issued under the new
	// write-only account key.
	if !shouldRenew && d.HasChange("account_key_pem_wo_version") {
		shouldRenew = true
	}
...
func resourceACMECertificateUpdate(d *schema.ResourceData, meta any) error {
	...
	// A change to account_key_pem_wo_version signals an account key rotation;
	// force a renewal so the certificate is re-issued under the new write-only
	// account key (see resourceACMECertificateCustomizeDiff).
	if !shouldRenew && d.HasChange("account_key_pem_wo_version") {
		shouldRenew = true
	}
	...
	if !shouldRenew {
		...
		// If a write-only private key was supplied, renew using it (it is not in
		// state, so expandCertificateResource cannot have populated it). Without
		// this, renewWithOptions would generate a fresh key for the new cert.
		if woKey := writeOnlyValue(d, "private_key_pem_wo"); woKey != "" {
			cert.PrivateKey = []byte(woKey)
		}
		...
		// As in Create, a write-only key is never persisted: clear it so
		// private_key_pem and certificate_p12 remain empty.
		if writeOnlyValue(d, "private_key_pem_wo") != "" {
...
func resourceACMECertificateRenewalInfoRefresh(
	...
	renewalInfoResp, err := certifier.GetRenewalInfo(context.TODO(), cert)
	if err != nil {
